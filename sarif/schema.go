...
type Artifact struct {
	...
	// LastModifiedTimeUtc time.Time `json:"lastModifiedTimeUtc,omitempty"`  // TODO : kill all times from definition
...
func (pb *PropertyBag) MarshalJSON() ([]byte, error) {
	m := map[string]interface{}{}
	for k, v := range pb.AdditionalProperties {
		m[k] = v
	}
...
func (pb *PropertyBag) UnmarshalJSON(data []byte) error {
	var m map[string]interface{}
	if err := json.Unmarshal(data, &m); err != nil {
		return err
	}
	pb.AdditionalProperties = make(map[string]interface{})
	for k, v := range m {
		switch k {
		case "tags":
			if tags, ok := v.([]interface{}); ok {
				pb.Tags = make([]string, len(tags))
				for i, tag := range tags {
					pb.Tags[i] = tag.(string)
				}
			}
		default:
			pb.AdditionalProperties[k] = v
		}
	}
	return nil
}
...
// Report Static Analysis Results Format (SARIF) Version 2.1.0-rtm.5 JSON Schema: a standard format for the output of static analysis tools.
type Report struct {
