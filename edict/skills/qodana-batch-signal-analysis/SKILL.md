...
1. Prepare one source mode.
    - PR mode: call `mcp__qodana__prepare_pr_analysis`.
...
3. Verify complete inspection coverage, then collect only accepted strong signals. Coverage bookkeeping is internal; do not create a signal, persisted result, or inbox placeholder for work items without signals.
4. Validate the signal array exactly once. An empty array is valid.
    - PR mode: call `mcp__qodana__validate_pr_signals` with `signalsJson`.
...
1. Record the complete ordered set of prepared work-item IDs before inspection.
