...
Start the task. Inspect the complete assigned human discussion or commit message, surrounding PR context when
applicable, and the exact before/after source and diff. Prefer local read-only Git; use available revision readers when
Git objects are absent. Never substitute the current checkout for historical evidence. If neither source can provide
