...
internal fun prepareGeneration(project: Path, output: Path, limit: Int, rules: Set<String>): Map<String, String> {
    ...
    val revision = commandOutput("git", "-C", project.toString(), "rev-parse", "HEAD")
    val mapping = selected.associate { spec ->
        ...
        directory.resolve("history.md").writeText("# Benchmark input\n\nImported unchanged from benchmark/$rule/specification.json at $revision.\n" +
            "Required labelled examples are source evidence. No commit/PR signals were fabricated. Optional examples are held out for scoring.\n")
        cluster to rule
    }
    ...
    output.resolve("prompt.txt").writeText("""
        ${'$'}edict_manager
        Run Kotlin managed generation for these Pending benchmark clusters: ${mapping.keys.joinToString()}.
        Source project: $project. State is managed exclusively through edict-mcp. Scratch: ${output.resolve("scratch")}.
        Each description.benchmarkSpecification contains the original required examples and description.
        This is generation-only: do not extract commits, distribute inbox entries, rename, merge or split clusters.
        Delegate edict-generation, which delegates edict-cluster-generation normally. Process clusters sequentially so
        the five-level review hierarchy fits six native-agent slots. Close completed native agents.

        Pass these benchmark evidence instructions to every generation worker and reviewer:
        There are no persisted commit/PR signals. Required positiveExamples and negativeExamples are labelled source
        evidence at Git revision $revision unless an example supplies another revision. For each required example create
        a transient scratch signal with its actual fileRevision/path/revision/expectedRanges, label, original description,
        and source.type SubmittedFeedback with the originating specification path. Delegate edict-code-example with
        example.write only on this cluster's synthetic-examples directory and no cluster.signal.write to make a small
        source-faithful example. Use the returned example ID for validation. Never persist these feedback records as
        commit/PR signals. Identical source locations with conflicting required labels must retain their conflicting
        evidence and a valid domain outcome. Do not silently relabel or remove fixtures.
