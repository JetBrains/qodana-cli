...
fun main(args: Array<String>) {
    try {
        ...
        when (command) {
            ...
            "mcp", "edict-mcp" -> {
                require(options.keys.all { it in listOf("project-dir", "state-dir", "http-port", "log-dir") }) { "Unknown MCP option" }
                ...
                Store(options["state-dir"]?.let(Path::of) ?: project.resolve(".edict")).use { store ->
                    ...
                    try {
                        val server = McpServer(store, logs = (options["log-dir"]?.let(Path::of) ?: project.resolve("log")).resolve("edict"))
                        if ("http-port" in options) server.serveHttp(options.getValue("http-port").toInt()).use { transport ->
    ...
    } catch (e: Exception) { System.err.println("edict: ${e.message}"); exitProcess(1) }
}
