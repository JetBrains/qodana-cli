...
class McpServer(private val store: Store, provider: ReviewProvider = ReviewClient(), private val logs: Path? = null) {
    ...
    init {
        ...
        tool(
            "edict_task_get",
            "Fetch authoritative assignment bound to your own token. Read returned prompt and assigned SKILL.md before starting. Required on every delegation including retry.",
            true,
            listOf("token")
