...
class Store(directory: Path) : AutoCloseable {
    ...
    @Synchronized fun delegate(token: String, taskId: String, operations: List<String>, scope: List<String>, prompt: String): Delegation {
        ...
        return Delegation(secret, taskId, task.skill, Registry.skillPath(task.skill),
            "\$managed-${task.skill}\nBefore loading any skill, call edict_task_get with token $secret to fetch your assigned task $taskId. " +
...
internal fun validArtifactPath(name: String) {
    require(validSourcePath(name) && name.split('/').none { it.endsWith('.') || it.endsWith(' ') }) { "Invalid relative artifact path '$name'" }
}
internal fun operation(name: String): String {
    ...
    if (p.size == 2 && p[0] == "inspections" && listOf(".candidate.kts", ".inspection.kts").any { p[1].endsWith(it) && identifier.matches(p[1].removeSuffix(it)) }) return "inspection.write"
    if (p.size >= 3 && p[0] == "clusters" && identifier.matches(p[1])) {
        ...
        if (p.size >= 5 && p[2] == "synthetic-examples" && identifier.matches(p[3]) &&
            (p.size == 5 && p[4] == "metadata.json" || p.size >= 6 && p[4] == "project")) return "example.write"
    }
    return ""
}
internal fun readable(name: String): Boolean = operation(name).isNotEmpty() ||
    runCatching { validArtifactPath(name) }.isSuccess && name.startsWith("plans/") && name.count { it == '/' } == 1 && name.endsWith(".json")
