...
internal fun ReviewClient.space(selection: ReviewSelection): List<PullRequest> {
    ...
    if (numbers.isEmpty()) {
        ...
        for (page in 0 until 1000) {
            val response = get("space", root, mapOf("\$skip" to "${page * 100}", "\$top" to "100", "state" to "Merged", "repository" to selection.repo,
                "sort" to "LastUpdatedDesc", "to" to selection.endDate, "\$fields" to "data(review(id,number,timestamp))")).body.jsonObject
            ...
            for (item in items) {
                ...
                if (item.number("timestamp") < start) { old = true; break }
    ...
    return numbers.mapNotNull { number ->
        val v = get("space", "$root/number:$number", mapOf("\$fields" to "id,number,state,timestamp,title,description,feedChannelId,branchPair(repository,isMerged,sourceBranchRef,targetBranchInfo(ref))")).body.jsonObject
        require(v.number("number").toInt() == number && v.text("state").isNotEmpty()) { "Wrong Space review or missing merge state" }
        ...
        val threads = spaceFeed(feed).mapNotNull { message ->
            ...
            if (details.text("className") != "CodeDiscussionAddedFeedEvent" || author.obj("details").obj("user").isEmpty() || isBot(author.text("name"), author.obj("details").text("className"))) return@mapNotNull null
            ...
            ReviewThread("space-$number-${message.text("id")}", "$spaceUrl/im/review/${urlPart(v.text("id"))}?message=${urlPart(message.text("id"))}&channel=${urlPart(feed)}",
                anchor.text("filename").removePrefix("/"), anchor.text("revision"), line, line, messages)
        }.sortedWith(compareBy({ it.messages.first().createdAt }, { it.threadId }))
        PullRequest(number, "$spaceUrl/p/${urlPart(selection.owner)}/reviews/$number", v.text("title"), v.text("description"),
...
private fun ReviewClient.spaceFeed(channel: String): List<JsonObject> {
    ...
    for (page in 0 until 1000) {
        val response = get("space", "/chats/messages/sync-batch", mapOf("channel" to "id:$channel", "batchInfo" to "{etag:$etag,batchSize:100}",
            "\$fields" to "data(chatMessage(id,text,created,details(className,codeDiscussion(id,channel(id),anchor(filename,line,oldLine,revision))),projectedItem(author(name,details(className,user(id)))))),etag,hasMore")).body.jsonObject
        response.array("data").map { it.obj("chatMessage") }.filter { it.isNotEmpty() }.forEach { all.putIfAbsent(it.text("id"), it) }
...
private fun ReviewClient.spaceDiscussion(channel: String): List<ReviewMessage> {
    ...
    var start = ""
    for (page in 0 until 1000) {
        val query = mutableMapOf("channel" to "id:$channel", "sorting" to "FromOldestToNewest", "batchSize" to "50",
