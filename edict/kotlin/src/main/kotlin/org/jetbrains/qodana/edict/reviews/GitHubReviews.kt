...
internal fun ReviewClient.github(selection: ReviewSelection): List<PullRequest> {
    ...
    if (numbers.isEmpty()) {
        ...
        for (page in 1..1000) {
            val response = get("github", "$root/pulls", mapOf("state" to "closed", "sort" to "updated", "direction" to "desc", "per_page" to "100", "page" to "$page"))
            val items = response.body.jsonArray.map { it.jsonObject }
            var old = false
            for (item in items) {
                val number = item.number("number").toInt()
                require(number > 0 && seen.add(number)) { "Invalid or repeated PR number" }
                if (Instant.parse(item.text("updated_at")).isBefore(start)) { old = true; break }
                if (item.text("merged_at").isNotEmpty() && selection.containsDate(Instant.parse(item.text("merged_at")).toEpochMilli())) numbers += number
    ...
    return numbers.mapNotNull { number ->
        ...
        val pr = PullRequest(number, v.text("html_url"), v.text("title"), v.text("body"), v.obj("base").text("sha"), v.obj("head").text("sha"), merged)
        ...
        val threads = comments.groupBy { it.number("in_reply_to_id").takeIf { id -> id > 0 } ?: it.number("id") }.mapNotNull { (id, group) ->
            val anchor = byId[id] ?: error("Missing root comment $id")
            val line = anchor.number("original_line").takeIf { it > 0 } ?: anchor.number("line")
            val start = anchor.number("original_start_line").takeIf { it > 0 } ?: anchor.number("start_line").takeIf { it > 0 } ?: line
            val messages = reviews.filter { it.number("id") == anchor.number("pull_request_review_id") && it.text("body").isNotBlank() }
            ...
            if (messages.isEmpty()) null else ReviewThread("github-$number-$id", "${pr.url}#discussion_r$id", anchor.text("path"), anchor.text("original_commit_id"), start.toInt(), line.toInt(), messages)
