...
object UnifiedDiff {
    ...
    fun parse(diff: String): Changes {
        ...
        diff.split('\n').forEachIndexed { index, line ->
            ...
            if (oldRemaining > 0 || newRemaining > 0) {
                ...
                line.startsWith("diff --git ") -> { beforePath = ""; afterPath = "" }
                line.startsWith("--- ") -> beforePath = unquotePath(line.substring(4).removeSuffix("\t")).removePrefix("a/")
                line.startsWith("+++ ") -> afterPath = unquotePath(line.substring(4).removeSuffix("\t")).removePrefix("b/")
                line.startsWith("@@") -> {
    ...
    private fun unquotePath(value: String): String {
        ...
        while (i < value.lastIndex) {
            val c = value[i++]
            if (c != '\\') { bytes.write(c.toString().toByteArray(Charsets.UTF_8)); continue }
