...
internal class ReviewProviderFixture(private val provider: String, repository: GitRepository) : AutoCloseable {
    ...
    val message =
        "Please replace Thread.sleep with an explicit worker completion signal. The final commit uses CountDownLatch.await instead."
    ...
    private fun response(exchange: HttpExchange): String {
        ...
        return when (path) {
            ...
            "/repos/owner/repo/pulls/7/comments" ->
                """[{"id":10,"path":"$historyPath","original_commit_id":"$historyBefore","original_line":5,"body":${
                    quote(
                        message
                    )
                },"created_at":"2026-09-20T12:00:00Z","user":{"login":"reviewer","type":"User"}}]"""
