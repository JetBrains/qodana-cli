...
internal fun verifyAgentLogs(logs: Path, plan: Plan) {
    ...
    listOf(full, short).forEach { text ->
        ...
        text.lineSequence()
            .forEach { assertTrue(it.codePointCount(0, it.length) <= 120, "Log must wrap at 120 characters") }
...
internal fun verifyRuntimeWorkers(sessions: Path, plan: Plan) {
                ...
                if (event.text("type") == "event_msg" && payload.text("type") == "item_completed") {
                    ...
                    if (item["exit_code"] == JsonPrimitive(0)) {
                        skillPath.findAll(item["command"].toString()).forEach { match ->
                            if (item.text("aggregated_output")
                                    .contains("name: ${match.groupValues[1]}")
                            ) skills += match.groupValues[1]
                        }
                    }
                    ...
                    if (item.text("server") == "edict-mcp") {
                        if (item.text("tool") == "edict_task_get" && !started && item.obj("result")
                                .flag("isError") != true
                        )
                            assignment = wireJson.decodeFromJsonElement<TaskAssignment>(
                                item.obj("result").getValue("structuredContent")
                            )
                ...
                if (event.text("type") == "response_item") when (payload.text("type")) {
                    "function_call", "custom_tool_call" -> {
                        ...
                        if (payload.text("name")
                                .substringAfterLast('.') == "spawn_agent"
                        ) spawnCalls += payload.text("call_id")
                    }

                    "function_call_output", "custom_tool_call_output" -> {
                        calls[payload.text("call_id")].orEmpty().forEach { skill ->
                            if (payload["output"].toString().contains("name: $skill")) skills += skill
                        }
                        if (payload.text("call_id") in spawnCalls) {
                            val output =
                                runCatching { wireJson.parseToJsonElement(payload.text("output")).jsonObject }.getOrNull()
                            listOf("agent_id", "task_name").mapNotNull { output?.text(it)?.takeIf(String::isNotEmpty) }
                                .forEach { spawned += it }
                        }
                    }
                }
    ...
    plan.tasks.forEach { task ->
        ...
        assertTrue(
            Registry.installedName(task.skill) in worker.skillsAtStart,
            "Worker must read its assigned skill before startup"
        )
        assertFalse("edict_manager" in worker.skills, "Worker must not execute the root manager skill")
        assertEquals(
            TaskAssignment(task.id, task.skill, Registry.skillPath(task.skill), task.prompt),
            worker.assignment,
            "Worker must fetch its exact assignment in its own session before startup"
        )
    }
}
