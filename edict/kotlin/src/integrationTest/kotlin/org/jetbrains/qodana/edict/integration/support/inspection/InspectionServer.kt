...
internal class InspectionServer private constructor(
    ...
    fun run(code: String, path: String, content: String): InspectionResult = decodeInspectionResult(client.call("run_inspection_kts", buildJsonObject {
    ...
    override fun close() {
        proxy.close()
        try { client.close() } finally { stopIde(process) }
    }
    ...
    companion object {
        fun start(workspace: IntegrationWorkspace): InspectionServer {
            ...
            Files.walk(workspace.project).use { paths -> paths.forEach { path ->
            ...
            val process = ProcessBuilder("/bin/sh", "./bazel.cmd", "run", "//build:mcp_server", "--",
                ...
                "mcpServer", "--project=$source", "--invocation-mode=direct", "--allowed-tools=${inspectionToolNames.joinToString(",")}")
            ...
            try {
                ...
                proxy = InspectionToolProxy(listed.map { it.jsonObject }, source, root.resolve("log/inspection-mcp.jsonl"), connected::call)
        ...
        private fun stopIde(process: Process) {
            ...
            runCatching { ProcessBuilder("/bin/kill", "-INT", process.pid().toString()).start().waitFor(5, TimeUnit.SECONDS) }
