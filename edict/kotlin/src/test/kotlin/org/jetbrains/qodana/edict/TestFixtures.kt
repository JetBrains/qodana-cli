...
internal const val afterSource = "class Equality {\n  boolean same(String a, String b) {\n    return java.util.Objects.equals(a, b);\n  }\n}\n"
...
internal fun gitFixture(directory: Path): GitRepository {
    ...
    git("add", "."); git("commit", "-qm", "Fix string equality: compare values rather than object identity, including nulls")
...
internal fun fixtureSignals(repository: GitRepository): List<Signal> = CommitSignalExtractor(repository) { commit, repo ->
    ...
    listOf(
        SignalFinding(SignalLabel.POSITIVE, fixturePath, listOf(SignalRange(3, 3)), "String reference equality does not compare string values"),
        SignalFinding(SignalLabel.NEGATIVE, fixturePath, listOf(SignalRange(3, 3)), "Objects.equals compares string values and safely handles nulls"),
    )
}.extract("HEAD")

internal fun Store.launch(parent: String, id: String, skill: String, operations: List<String>, scope: List<String>): Delegation {
    val worker = delegate(parent, id, operations, scope, "\$managed-$skill\nRead /skills/${Registry.skillPath(skill)} and perform the assigned bounded task.")
...
internal fun Store.batch(): Pair<PlanCreation, Delegation> {
    val created = createPlan("Extract signals from the latest commit", listOf(Step("edict-batch-signal-analysis", "Extract latest correction")))
    return created to launch(created.token, created.plan.tasks.single().id, "edict-batch-signal-analysis", listOf("inbox.write"), listOf("inbox"))
}
