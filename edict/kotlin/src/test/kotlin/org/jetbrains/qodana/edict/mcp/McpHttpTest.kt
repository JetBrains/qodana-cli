...
class McpHttpTest {
    ...
    private val createPlan =
        """{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"edict_plan_create","arguments":{"request":"Extract","steps":[{"skill":"edict-batch-signal-analysis","title":"Commit"}]}}}"""
    private val ping = """{"jsonrpc":"2.0","id":1,"method":"ping"}"""

    private fun HttpClient.post(
        url: String,
        body: String = createPlan,
    ...
    @Test
    fun `untrusted and malformed origins cannot claim manager authority`() {
            ...
            McpServer(store).serveHttp().use { transport ->
                HttpClient.newHttpClient().use { client ->
                    ...
                    listOf(
                        ...
                        assertEquals(
                            403,
                            client.post(transport.url, origins = origins).statusCode(),
                            origins.toString()
                        )
                    ...
                    assertEquals(
                        ...
                        client.post(
                            transport.url,
                            origins = listOf("https://untrusted.example"),
    ...
    @Test
    fun `non JSON posts cannot mutate state`() {
        Store(directory.resolve("state")).use { store ->
            McpServer(store).serveHttp().use { transport ->
                HttpClient.newHttpClient().use { client ->
                    listOf(
                    ...
                    assertEquals(
                        200,
                        client.post(transport.url, contentType = "application/json; charset=utf-8").statusCode()
                    )
    ...
    @Test
    fun `native and same origin clients retain HTTP access`() {
        Store(directory.resolve("state")).use { store ->
            McpServer(store).serveHttp().use { transport ->
                HttpClient.newHttpClient().use { client ->
                    val origin = transport.url.removeSuffix("/mcp")
                    listOf(
                        emptyList(),
                        listOf(origin),
                        listOf(origin.replace("127.0.0.1", "localhost"))
                    ).forEach { origins ->
