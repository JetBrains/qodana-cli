...
class SignalValidationTest {
    ...
    @Test fun `validate both sides of real commit and reject forged source evidence`() {
        ...
        assertFails { repository.validateEvidence(signal.copy(fileRevision = signal.fileRevision.copy(expectedRanges = listOf(SignalRange(3, 999))))) }
    }

    @Test fun `diff parser validates counts and sides for renames quoted paths additions and deletions`() {
        val diff = "diff --git a/old b/new\n--- a/old\n+++ b/new\n@@ -1,2 +1,2 @@\n--- source text\n+++ source text\n context\n"
        ...
        val quoted = UnifiedDiff.parse("--- \"a/caf\\303\\251.java\"\n+++ \"b/caf\\303\\251.java\"\n@@ -1 +1 @@\n-old\n+new\n")
        ...
        listOf("not a diff", diff.replace("-1,2", "-1,3"), diff + "+excess\n", diff.replace(" context\n", ""), "@@ -1 +1 @@\n-a\n+b\n").forEach {
