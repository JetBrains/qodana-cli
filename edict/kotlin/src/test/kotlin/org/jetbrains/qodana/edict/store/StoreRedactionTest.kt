...
class StoreRedactionTest {
    ...
    @Test fun `redaction finds embedded adjacent and revoked capabilities without hiding unrelated digests`() {
    ...
    @Test fun `embedded capabilities cannot be saved in task titles or results`() {
        Store(directory.resolve("state")).use { store ->
            val (manager, batch) = store.batch()
    ...
    @Test fun `rejected arguments do not leak embedded credentials into MCP or agent logs`() {
        Store(directory.resolve("state")).use { store ->
            val (manager, batch) = store.batch()
            ...
            Files.list(logs).use { paths -> paths.forEach { file ->
