...
func TestSetDefaultCommandIfNeeded(t *testing.T) {
	tests := []struct {
		...
		{"no args adds scan", []string{"qodana", "-i", "."}, []string{"scan", "-i", "."}},
		{"help flag unchanged", []string{"qodana", "--help"}, nil},
		{"version flag unchanged", []string{"qodana", "-v"}, nil},
		{"scan command unchanged", []string{"qodana", "scan", "-i", "."}, nil},
		{"init command unchanged", []string{"qodana", "init"}, nil},
	}
	for _, tt := range tests {
		t.Run(
			tt.name, func(t *testing.T) {
				...
				// The function modifies rootCmd.SetArgs if needed
			},
		)
	}
}
