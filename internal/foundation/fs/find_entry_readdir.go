...
// findEntryByReaddir looks up a directory entry by name, returning the actual
// on-disk name. It scans directory entries via Readdirnames, which requires
// READ permission on the parent directory (not just EXECUTE).
//
// This is the portable fallback used on platforms without dedicated OS APIs
// (e.g. BSDs) and as a fallback on Linux when /proc is unavailable.
func findEntryByReaddir(dir, name string) (result string, err error) {
	d, err := os.Open(dir)
	if err != nil {
		return "", err
	}
