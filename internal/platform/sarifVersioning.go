...
func getRepositoryUri(pwd string) (string, error) {
	uri, stderr, ret, err := exec.ExecRedirectOutput(pwd, "git", "ls-remote", "--get-url")
	if err != nil {
		return "", err
	}
	if ret != 0 {
...
func getRevisionId(pwd string) (string, error) {
	rev, _, ret, err := exec.ExecRedirectOutput(pwd, "git", "rev-parse", "HEAD")
	if err != nil {
		return "", err
	}
	if ret != 0 {
...
func getBranchName(pwd string) (string, error) {
	...
	branch, _, ret, err := exec.ExecRedirectOutput(pwd, "git", "rev-parse", "--abbrev-ref", "HEAD")
	if err != nil {
		return "", err
	}
	if ret == 128 {
		// this approach covers some corner cases, notably when no commits exist
		branch, _, ret, err = exec.ExecRedirectOutput(pwd, "git", "symbolic-ref", "--short", "HEAD")
		if err != nil {
			return "", err
		}
	}
...
func getLastAuthorName(pwd string) string {
	name, _, ret, err := exec.ExecRedirectOutput(pwd, "git", "log", "-1", "--pretty=format:%an")
	if err != nil || ret != 0 {
		return ""
	}
...
func getAuthorEmail(pwd string) string {
	email, _, ret, err := exec.ExecRedirectOutput(pwd, "git", "log", "-1", "--pretty=format:%ae")
	if err != nil || ret != 0 {
		return ""
	}
